...
const SOURCE_DEFAULT_VERTICAL_FLIPS = new Set(['polycam', 'insta360']);
...
const state = {
  ...
  lastLoadedName: '',
  lastLoadedSource: null,   // provider used for source-specific model orientation
...
const translations = {
  zh: {
    // Toolbar
    'url-placeholder': '粘贴 Kiri Engine、Polycam、Luma 或 Insta360 分享链接...',
    ...
    'kiri-visit-link': '→ 前往 Kiri Engine',
    'kiri-intro-content': 'Kiri Engine 将真实场景转化为可分享的 3D 内容。KIRI Maker 可导入 Kiri Engine、Polycam、Luma 和 Insta360 分享链接，并提供粒子特效、可编辑的 3DGS 裁剪与橡皮擦、手势控制及自定义运镜。',
    // Welcome Hint & Loading
    'welcome-title': 'KIRI 自定义特效运镜工具',
    'welcome-desc': '在下方粘贴 Kiri Engine、Polycam、Luma 或 Insta360 分享链接，即可体验粒子模式、可编辑 3D 实景、手势控制与自定义运镜。',
    'landing-url-placeholder': '粘贴 Kiri Engine、Polycam、Luma 或 Insta360 分享链接，无链接直接点击加载体验',
    'loading-init': '加载中...',
    ...
    // Alerts/Toasts
    'enter-valid-url': '请输入 Kiri Engine、Polycam、Luma 或 Insta360 分享链接',
    'enter-valid-url-2': '请输入有效的 Kiri Engine、Polycam、Luma 或 Insta360 分享链接',
  ...
  en: {
    // Toolbar
    'url-placeholder': 'Paste a Kiri Engine, Polycam, Luma, or Insta360 share URL...',
    ...
    'camera-panel-subtitle': 'Please adjust your view; the initial position will serve as the first frame of the built-in flight preset. During export, you can toggle Particle Mode and play particle effects, which will be fully recorded.',
    ...
    'kiri-intro-content': 'Kiri Engine turns real-world scenes into shareable 3D content. KIRI Maker imports Kiri Engine, Polycam, Luma, and Insta360 share links for particle effects, editable 3DGS crop and eraser tools, gesture control, and custom camera paths.',
    // Welcome Hint & Loading
    'welcome-title': 'KIRI Custom Effects & Camera Tool',
    'welcome-desc': 'Paste a Kiri Engine, Polycam, Luma, or Insta360 share URL below to explore Particle Mode, editable 3D Reality, gesture control, and custom camera paths.',
    'landing-url-placeholder': 'Paste a Kiri Engine, Polycam, Luma, or Insta360 share link, or load without one to try the demo',
    ...
    // Alerts/Toasts
    'enter-valid-url': 'Please enter a Kiri Engine, Polycam, Luma, or Insta360 share URL',
    'enter-valid-url-2': 'Please enter a valid Kiri Engine, Polycam, Luma, or Insta360 share URL',
...
function applyTranslations(lang) {
  ...
  const kiriTitle = document.getElementById('kiri-intro-title');
  if (kiriTitle) kiriTitle.querySelector('.kiri-intro-title-text').textContent = dict['kiri-intro-title'];
  const kiriVisitLink = document.getElementById('kiri-visit-link');
...
async function loadFromUrl() {
  ...
  try {
    ...
    // A share page may be cached with an expired signed model URL.
...
function detectModelFileFormat(buffer, explicitFormat = '') {
  ...
  // Signed resources can keep an outdated extension. Trust the payload
  // signature before the URL-derived format so they enter the right parser.
...
async function processBuffer(buffer, name, isFreshLoad = false, options = {}) {
  const source = options.source ?? state.lastLoadedSource;
  ...
  // Apply provider defaults and the user's vertical-flip state.
  ...
  try {
    ...
    const finishSplatMeshLoad = (mesh) => {
      ...
      // Align Spark SplatMesh exactly with the particle renderer.
      ...
      if (state.initialCameraPosition) {
        ...
        // Apply provider-specific model orientation.
...
const updateKiriPosition = () => {
  ...
  // KIRI Engine card above the top-most active control (including the progress bar)
...
function updateRendererUI() {
  ...
  // Show the progress control only in Particle Mode after a model is loaded.
  if (dom.progressControl) {
...
function updateKeyframeUI() {
  ...
  state.keyframes.forEach((kf, i) => {
    ...
    item.tabIndex = 0;
    item.addEventListener('click', () => jumpToKeyframe(i));
...
function setupEventListeners() {
  ...
  if (dom.btnFlipX) {
    dom.btnFlipX.addEventListener('click', () => {
      ...
      // Reapply provider defaults plus the user-controlled vertical flip.
