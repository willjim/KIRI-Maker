...
import { extractPLYFromUrl, downloadPLY } from './kiriLoader.js';
...
const SOURCE_DEFAULT_MODEL_ROTATIONS = Object.freeze({
  luma: Object.freeze({ x: 0, y: 0, z: Math.PI }),
  polycam: Object.freeze({ x: Math.PI / 2, y: 0, z: 0 }),
});
const SOURCE_DEFAULT_VERTICAL_FLIPS = new Set(['polycam']);
const SUPPORTED_SHARE_HOSTS = new Set([
  'www.remy3d.cn',
  'remy3d.cn',
  'www.kiriengine.app',
  'kiriengine.app',
...
function isSupportedShareUrl(value) {
  try {
    ...
    return parsed.protocol === 'https:' && SUPPORTED_SHARE_HOSTS.has(parsed.hostname);
  } catch {
    return false;
  }
}

function setTablerIcon(container, iconName, { spinning = false } = {}) {
  const svg = container?.matches?.('.tabler-icon')
    ? container
    : container?.querySelector?.('.tabler-icon');
  const use = svg?.querySelector('use');
  if (!use) return;
  use.setAttribute('href', `#ti-${iconName}`);
  svg.classList.toggle('is-spinning', spinning);
}

function createTablerIcon(iconName, className = '') {
  const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('class', `tabler-icon${className ? ` ${className}` : ''}`);
...
const state = {
  ...
  splatMesh: null,
  splatPivot: null,
  lang: 'en', // English is the default while the language switcher is hidden.
  isModelLoaded: false,
  modelRotationPaused: false,
  ...
  initialCameraHeightScale: 1.0, // Per-share vertical camera adjustment
  lastLoadedBuffer: null,   // cache raw buffer for fast re-parsing on settings change
  lastLoadedName: '',
  lastLoadedSource: null,   // share-link provider, used for source-specific orientation
...
function getModelRotation() {
  const sourceRotation = SOURCE_DEFAULT_MODEL_ROTATIONS[state.lastLoadedSource]
    || { x: 0, y: 0, z: 0 };
  const sourceVerticalFlipX = SOURCE_DEFAULT_VERTICAL_FLIPS.has(state.lastLoadedSource)
    ? Math.PI
    : 0;
  const userFlipRotationX = state.xFlipped ? 0 : Math.PI;
  return new THREE.Euler(
    sourceRotation.x + sourceVerticalFlipX + userFlipRotationX,
    sourceRotation.y,
    sourceRotation.z,
    'XYZ'
  );
...
function applyModelRotation(object3D) {
  object3D.rotation.copy(getModelRotation());
}

function alignSplatMesh(mesh, scale, center) {
  ...
  centeredOffset.applyEuler(rotation);
  mesh.position.copy(centeredOffset);
}

function updateRotationToggleUI() {
  const dict = translations[state.lang];
  const text = state.modelRotationPaused
    ? (dict?.['btn-resume-rotation'] || 'Resume Rotation')
    : (dict?.['btn-stop-rotation'] || 'Stop Rotation');

  if (dom.btnToggleRotation) {
    const textSpan = dom.btnToggleRotation.querySelector('.btn-text');
    if (textSpan) textSpan.textContent = text;
    dom.btnToggleRotation.title = text;
    dom.btnToggleRotation.classList.toggle('active', state.modelRotationPaused);
    dom.btnToggleRotation.setAttribute('aria-pressed', String(state.modelRotationPaused));
  }

  if (dom.btnMobileToggleRotation) {
    dom.btnMobileToggleRotation.title = text;
    dom.btnMobileToggleRotation.setAttribute('aria-label', text);
    dom.btnMobileToggleRotation.setAttribute('aria-pressed', String(state.modelRotationPaused));
    dom.btnMobileToggleRotation.classList.toggle('is-paused', state.modelRotationPaused);
    dom.btnMobileToggleRotation.classList.toggle('visible', state.isModelLoaded);
    requestAnimationFrame(updateMobileRotationButtonPosition);
  }
}
...
function toggleModelRotation() {
  ...
  state.modelRotationPaused = !state.modelRotationPaused;
  if (state.particleSystem) {
    state.particleSystem.autoRotate = !state.modelRotationPaused
  ...
  showToast(
    state.modelRotationPaused ? 'model-rotation-stopped' : 'model-rotation-resumed',
    'success'
  );
}
// ============================================================
...
function cacheDom() {
  dom = {
    ...
    kiriIntroPanel: document.getElementById('kiri-intro-panel'),
    ...
    landingBtnRandom: document.getElementById('landing-btn-random'),
...
function onResize() {
  ...
  updateToastPosition();
  updateMobileRotationButtonPosition();
}
...
const translations = {
  zh: {
    // Toolbar
    'url-placeholder': '粘贴 Kiri Engine、Polycam 或 Luma 分享链接...',
    'btn-load-title': '从链接加载模型',
    'btn-load-text': '加载',
    'btn-random-model-title': '随机加载示例模型',
    ...
    // KIRI Engine Intro
    'kiri-intro-title': '关于 Kiri Engine',
    'kiri-visit-link': '前往 Kiri Engine 官网',
    'kiri-intro-content': 'Kiri Engine 可将真实世界转化为可分享的 3D 内容。本工具支持导入 Kiri Engine、Polycam 和 Luma 的 3DGS 分享链接，体验自定义运镜、发光粒子特效与手势控制。',
    // Welcome Hint & Loading
    'welcome-title': 'KIRI Maker 自定义特效运镜工具',
    'welcome-desc': '在下方输入框中粘贴 <a href="https://www.kiriengine.app/" target="_blank" rel="noopener" class="desc-link">Kiri Engine</a>、Polycam 或 Luma 分享链接，即可开始 3D 粒子和 3DGS 实景体验。',
    'landing-url-placeholder': '粘贴 Kiri Engine、Polycam 或 Luma 分享链接...',
    'loading-init': '加载中...',
    ...
    // Alerts/Toasts
    'enter-valid-url': '请输入支持的 3D 分享链接',
    'enter-valid-url-2': '请输入有效的 Kiri Engine、Polycam 或 Luma 分享链接',
    ...
    'model-rotation-stopped': '模型旋转已停止',
    'model-rotation-resumed': '模型旋转已恢复',
  ...
  en: {
    // Toolbar
    'url-placeholder': 'Paste a Kiri Engine, Polycam, or Luma share URL...',
    'btn-load-title': 'Load model from URL',
    'btn-load-text': 'Load',
    'btn-random-model-title': 'Load a random sample model',
    ...
    // KIRI Engine Intro
    'kiri-intro-title': 'About Kiri Engine',
    'kiri-visit-link': 'Visit Kiri Engine',
    'kiri-intro-content': 'Kiri Engine turns real-world scenes into shareable 3D content. This tool imports 3DGS share links from Kiri Engine, Polycam, and Luma for custom camera paths, glowing particle effects, and gesture control.',
    // Welcome Hint & Loading
    'welcome-title': 'KIRI Custom Effects & Camera Tool',
    'welcome-desc': 'Paste a <a href="https://www.kiriengine.app/" target="_blank" rel="noopener" class="desc-link">Kiri Engine</a>, Polycam, or Luma share URL below to explore Particle Mode and 3D Reality.',
    'landing-url-placeholder': 'Paste a Kiri Engine, Polycam, or Luma share URL here...',
    ...
    // Alerts/Toasts
    'enter-valid-url': 'Please enter a supported 3D share URL',
    'enter-valid-url-2': 'Please enter a valid Kiri Engine, Polycam, or Luma share URL',
    ...
    'gesture-tracking-active': 'Hand tracking active! Open/close hand to morph, use two fists to scale the model, and hold the Victory gesture for 0.7 seconds to toggle mode',
    ...
    'model-rotation-stopped': 'Model rotation stopped',
    'model-rotation-resumed': 'Model rotation resumed',
...
function applyTranslations(lang) {
  ...
  // Welcome Hint
  if (dom.welcomeHint) {
    const titleText = dom.welcomeHint.querySelector('.landing-title-text');
    if (titleText) titleText.textContent = dict['welcome-title'];
    ...
    if (dom.landingBtnRandom) {
      dom.landingBtnRandom.title = dict['btn-random-model-title'];
      dom.landingBtnRandom.setAttribute('aria-label', dict['btn-random-model-title']);
    }
  ...
  if (guidePalm) guidePalm.querySelector('.gesture-guide-text').textContent = dict['guide-palm'];
  const guideFist = document.getElementById('guide-fist');
  if (guideFist) guideFist.querySelector('.gesture-guide-text').textContent = dict['guide-fist'];
  const guideVictory = document.getElementById('guide-victory');
  if (guideVictory) guideVictory.querySelector('.gesture-guide-text').textContent = dict['guide-victory'];
  const guideZoom = document.getElementById('guide-zoom');
  if (guideZoom) guideZoom.querySelector('.gesture-guide-text').textContent = dict['guide-zoom'];
  const guidePointing = document.getElementById('guide-pointing');
  if (guidePointing) guidePointing.querySelector('.gesture-guide-text').textContent = dict['guide-pointing'];
  
  // Translate KIRI Engine Intro panel
  const kiriTitle = document.querySelector('#kiri-intro-title .kiri-intro-title-text');
  if (kiriTitle) kiriTitle.textContent = dict['kiri-intro-title'];
  const kiriVisitLink = document.querySelector('#kiri-visit-link .kiri-visit-link-text');
  if (kiriVisitLink) kiriVisitLink.textContent = dict['kiri-visit-link'];
  const kiriContent = document.getElementById('kiri-intro-content');
  if (kiriContent) kiriContent.textContent = dict['kiri-intro-content'];
...
function updateToastPosition() {
  ...
  if (isMobileViewport()) {
    dom.toastContainer.style.removeProperty('top');
    return;
  }

  const toolbarRect = dom.toolbar?.getBoundingClientRect();
  const top = toolbarRect && toolbarRect.width > 0 && toolbarRect.height > 0
    ? toolbarRect.bottom + 12
    : 20;
  dom.toastContainer.style.top = `${Math.round(top)}px`;
}
...
// Model Loading
// ============================================================
const RANDOM_MODEL_URLS = Object.freeze([
  'https://www.kiriengine.app/share/3dgs?taskId=2058482445153468416',
  'https://www.kiriengine.app/share/3dgs?taskId=2038871173617418240',
  'https://www.kiriengine.app/share/3dgs?taskId=2059710987891638272',
  'https://www.kiriengine.app/share/3dgs?taskId=2038730854238257152',
  'https://www.kiriengine.app/share/3dgs?taskId=2040589854047207424'
]);
const LOWER_DEFAULT_CAMERA_TASK_ID = '2040589854047207424';

function getInitialCameraHeightScale(shareUrl) {
  try {
    return new URL(shareUrl).searchParams.get('taskId') === LOWER_DEFAULT_CAMERA_TASK_ID
      ? 0.15
      : 1.0;
  } catch {
    return 1.0;
...
/**
 * Load a model from a supported public share URL.
 */
async function loadFromUrl() {
  const url = dom.urlInput.value.trim();
  if (!url) {
    showToast(t('enter-valid-url'), 'error');
    return;
  }
  if (!isSupportedShareUrl(url)) {
    showToast(t('enter-valid-url-2'), 'error');
    return;
  }
  ...
  try {
    ...
    const { plyUrl, pcdUrl, splatUrl, name, source, initialCameraPosition } = result;
    state.initialCameraPosition = initialCameraPosition || null;
    state.initialCameraHeightScale = getInitialCameraHeightScale(url);
    if (!plyUrl && !pcdUrl && !splatUrl) {
      throw new Error('No 3D model file URL found');
    }
    // Step 2: Prefer the Gaussian PLY, then try Splat and point-cloud fallbacks.
    updateLoadingProgress(0.2, `Downloading: ${name}...`);
    let buffer = null;
    const urlsToTry = [plyUrl, splatUrl, pcdUrl].filter(Boolean);
    ...
    await processBuffer(buffer, name, true, source);
  } catch (error) {
    hideLoading();
...
async function loadFromFile(file) {
  ...
  state.initialCameraHeightScale = 1.0;
  showLoading(`Loading: ${file.name}`);
  try {
    const buffer = await file.arrayBuffer();
    await processBuffer(buffer, file.name.replace(/\.(?:ply|splat)$/i, ''), true, null);
  } catch (error) {
    hideLoading();
 ...
 */
async function processBuffer(buffer, name, isFreshLoad = false, source = state.lastLoadedSource) {
  // Cache raw buffer for quick reloading on settings changes
  state.lastLoadedBuffer = buffer;
  state.lastLoadedName = name;
  state.lastLoadedSource = source || null;
  if (isFreshLoad) {
    state.modelRotationPaused = false;
    updateRotationToggleUI();
  }
  ...
  // Apply the source-specific default orientation and the user's X-axis flip.
  if (state.particleSystem.points) {
    applyModelRotation(state.particleSystem.points);
  }
  ...
  try {
    const currentSplatMesh = new state.SplatMeshClass({
      ...
      onLoad: (mesh) => {
        ...
        // Align Spark SplatMesh position, scale and orientation with Particle System.
        alignSplatMesh(mesh, scale, center);
        ...
        if (state.initialCameraPosition) {
          ...
          camPos.applyEuler(getModelRotation());
          camPos.y *= state.initialCameraHeightScale;
          ...
          state.camera.position.set(
            0,
            0.15 * targetDistance * state.initialCameraHeightScale,
            targetDistance
          );
        }
        state.controls.target.set(0, 0, 0);
...
// Gesture Control
// ============================================================
function updateMobileRotationButtonPosition() {
  const button = dom.btnMobileToggleRotation;
  if (!button) return;

  if (!isMobileViewport() || !state.isModelLoaded || !button.classList.contains('visible')) {
    button.style.removeProperty('bottom');
    return;
  }

  const anchors = [];
  const addVisibleAnchor = (element, { allowTransparent = false } = {}) => {
    if (!element) return;
    ...
    if (
      ...
      && (allowTransparent || Number(style.opacity || 1) > 0.05)
      && rect.width > 0
      && rect.height > 0
    ) {
      anchors.push(rect.top);
    }
  };

  const progressSuppressed = document.body.classList.contains('camera-path-active')
    || document.body.classList.contains('settings-panel-active');
  if (dom.progressControl?.classList.contains('visible') && !progressSuppressed) {
    // Reserve the progress panel's space before its delayed fade-in starts, so
    // the rotation control completes its upward move before the panel appears.
    addVisibleAnchor(dom.progressControl, { allowTransparent: true });
  }
  if (document.body.classList.contains('settings-panel-active')) {
    addVisibleAnchor(dom.settingsPanel);
  }
  if (document.body.classList.contains('camera-path-active')) {
    addVisibleAnchor(dom.cameraPathPanel);
  }
  addVisibleAnchor(dom.kiriIntroPanel);
  if (document.body.classList.contains('preview-mode-active')) {
    addVisibleAnchor(dom.previewControls);
  } else if (document.body.classList.contains('recording-mode-active')) {
    addVisibleAnchor(dom.recordingControls);
  } else {
    addVisibleAnchor(dom.toolbar);
  }
  ...
  const anchorTop = Math.min(...anchors);
  const bottom = Math.max(12, window.innerHeight - anchorTop + 12);
  button.style.bottom = `${Math.round(bottom)}px`;
}

const updateKiriPosition = () => {
  updateMobileRotationButtonPosition();
  const panel = document.getElementById('kiri-intro-panel');
  ...
  // Preview/export controls are fixed to the bottom edge on phones. Keep the
  // KIRI Engine card above the top-most active control so landscape screens and
  // safe-area insets cannot make the controls overlap it.
  ...
  if (webcamActive) {
    ...
    panel.style.setProperty('bottom', (viewH - wcTop + gap) + 'px', 'important');
  } else if (progressActive) {
    const pcRect = dom.progressControl.getBoundingClientRect();
    const viewH = window.innerHeight;
    panel.style.setProperty('bottom', (viewH - pcRect.top + gap) + 'px', 'important');
  } else if (isMobileViewport() && state.isModelLoaded && dom.toolbar) {
    const toolbarRect = dom.toolbar.getBoundingClientRect();
    if (toolbarRect.width > 0 && toolbarRect.height > 0) {
      panel.style.setProperty('bottom', `${window.innerHeight - toolbarRect.top + gap}px`, 'important');
    } else {
...
const updateKiriPositionDeferred = () => {
  updateKiriPosition();
  // Re-position after CSS transition completes (~450ms)
  setTimeout(updateKiriPosition, 500);
};

function suppressKiriPanelTransition() {
  const panel = dom.kiriIntroPanel;
  if (!panel || !isMobileViewport()) return;
  panel.classList.add('transition-suppressed');
  requestAnimationFrame(() => {
    requestAnimationFrame(() => panel.classList.remove('transition-suppressed'));
  });
}
...
async function toggleGestureControl() {
  if (state.isGestureActive) {
    ...
    updateKiriPositionDeferred();
    const gIconSpan = dom.btnGesture.querySelector('.btn-icon-symbol');
    const gTextSpan = dom.btnGesture.querySelector('.btn-text');
    setTablerIcon(gIconSpan, 'hand-stop');
    ...
    setTablerIcon(dom.gestureIcon, 'hand-stop');
  ...
  state.gestureControl.onStatusChange = (status, message) => {
    switch (status) {
      ...
      case 'active':
        ...
        const actIcon = dom.btnGesture.querySelector('.btn-icon-symbol');
        const actText = dom.btnGesture.querySelector('.btn-text');
        setTablerIcon(actIcon, 'circle-check');
  ...
  const gIconSpan = dom.btnGesture.querySelector('.btn-icon-symbol');
  const gTextSpan = dom.btnGesture.querySelector('.btn-text');
  setTablerIcon(gIconSpan, 'loader-2', { spinning: true });
...
function animate() {
  ...
  if (state.particleSystem) {
    ...
    state.particleSystem.autoRotate = !state.modelRotationPaused && !cameraFlowLocksRotation;
  ...
  if (state.isGestureActive && state.gestureControl && !state.previewActive && !state.recordingActive) {
    ...
    // Toggle Spark 3DGS mode via the Victory gesture (state transition trigger with 0.7s hold threshold)
...
function updateRendererUI() {
  ...
  const iconSpan = dom.btnToggleSpark.querySelector('.btn-icon-symbol');
  ...
  if (dom.progressControl) {
    ...
    if (typeof updateKiriPosition === 'function') {
      updateKiriPositionDeferred();
    }
  }
}
...
function toggleCameraMode() {
  ...
  if (state.cameraModeActive) {
    ...
    // Restore auto-rotation only when the user has not paused it.
    if (state.particleSystem) {
      state.particleSystem.autoRotate = !state.modelRotationPaused;
      state.particleSystem.isGestureActive = state.isGestureActive;
    }
...
function updateKeyframeUI() {
  ...
  state.keyframes.forEach((kf, i) => {
    ...
    item.classList.toggle('selected', i === state.selectedKeyframeIndex);
    
    const label = document.createElement('button');
    label.type = 'button';
    label.className = 'keyframe-view-button';
    label.textContent = `${state.lang === 'zh' ? '视角' : 'Viewpoint'} ${i + 1}`;
    label.title = state.lang === 'zh' ? '恢复此关键帧视角' : 'Restore this keyframe viewpoint';
    label.setAttribute('aria-pressed', String(i === state.selectedKeyframeIndex));
    ...
    removeBtn.setAttribute('aria-label', 'Remove keyframe');
    removeBtn.appendChild(createTablerIcon('x'));
...
function restoreKeyframe(index) {
  ...
  dom.keyframeList.querySelectorAll('.keyframe-item').forEach((item, itemIndex) => {
    const selected = itemIndex === index;
    item.classList.toggle('selected', selected);
    item.querySelector('.keyframe-view-button')?.setAttribute('aria-pressed', String(selected));
  });
  state.camera.position.copy(keyframe.position);
  state.controls.target.copy(keyframe.target);
  state.camera.lookAt(keyframe.target);
...
function setupEventListeners() {
  ...
  if (dom.landingBtnRandom) {
    dom.landingBtnRandom.addEventListener('click', loadRandomModel);
  }
  ...
  const updatePlayButtonUI = () => {
    const icon = dom.btnPlayScatter ? dom.btnPlayScatter.querySelector('.scatter-play-icon') : null;
    if (!icon) return;
    icon.classList.toggle('is-reversed', scatterDirection !== 'forward');
    dom.btnPlayScatter.title = scatterDirection === 'forward'
      ? (state.lang === 'zh' ? '播放粒子消散效果' : 'Play scatter effect')
      : (state.lang === 'zh' ? '反向播放粒子聚合效果' : 'Reverse gather effect');
  };
  ...
  // KIRI Engine Intro Panel minimize listener
  const btnMinimizeKiri = document.getElementById('btn-minimize-kiri');
  const kiriIntroPanel = document.getElementById('kiri-intro-panel');
  if (btnMinimizeKiri && kiriIntroPanel) {
    btnMinimizeKiri.addEventListener('click', () => {
      kiriIntroPanel.classList.add('minimized');
      updateKiriPositionDeferred();
      setTimeout(() => {
        kiriIntroPanel.style.display = 'none';
        updateKiriPositionDeferred();
      }, 220);
    });
  }
  ...
  if (dom.btnFlipX) {
    dom.btnFlipX.addEventListener('click', () => {
      ...
      // Preserve the source-specific orientation while toggling the X-axis flip.
  ...
  [dom.btnToggleRotation, dom.btnMobileToggleRotation].forEach((button) => {
    button?.addEventListener('click', toggleModelRotation);
  });
...
function init() {
  ...
  // Update KIRI Engine panel position on startup
  if (typeof updateKiriPosition === 'function') {
    updateKiriPosition();
  }
